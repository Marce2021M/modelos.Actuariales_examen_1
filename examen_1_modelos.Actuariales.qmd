...
flujos_utilidad <- function(kTemp){
  ...
  if (kTemp %in% c(1,2,3,4)){
    ingreso <- (gastoPropPrima(kTemp)*primaRecargada-gastoFijoPrima(kTemp))*(1+iTasaEstimada)*tablaMortalidadEstimada$lx[xEdad+kTemp] + tablaValoresPolizaAsociadosRecargada$Vx[kTemp]*(1+iTasaEstimada)*tablaMortalidadEstimada$lx[xEdad+kTemp]
    ...
    resultado <- 1000*(ingreso-egreso-reserva)/tablaMortalidadEstimada$lx[xEdad+1]} else {
