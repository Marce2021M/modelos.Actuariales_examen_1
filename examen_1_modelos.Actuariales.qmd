...
Dados los valores observados por la aseguradora tendremos los siguientes gastos representados por la siguiente gráfica.

```{r, echo=FALSE}

#ggplot(valoresObservados_costosPromedios)) +
#  geom_line() +
#  geom_point() +
#  labs(title = "Tabla de mortalidad", x = "Supervivencia", y = "px") 
```
...
La tasa base será del `r itasa*100`%, porque actualmente existe la expectativa de disminución en la tasa de interés. Esto debido a que la inflación está disminuyendo, y la política de BM es bajar las tasas de interés una vez controlada la inflación. Además, como el seguro tiene una vigencia de `r n` años esperamos que disminuya aún más manteniendose en promedio del `r itasa*100`%.
